...
from app.pipeline.cluster import ClusterConfig, Constraints, ReclusterResult, recluster
...
class JobHandlers:
  ...
  def _handle_classify(self, request: ClassifyRequest) -> ClassifyResult:
    ...
    saved, result = self._recluster_and_save(request.event_id, event)
    # TODO(CHMO-165): request.options(눈감음/흔들림 제외 토글)는 품질 판정 파이프라인 미구현으로 아직 무시된다
    return self._assemble_result(request.job_id, saved, result, common_album=common_album, failed_images=failed_images)
  ...
  def _handle_feedback(self, message: MergeFeedback | SplitFeedback | ReassignFeedback) -> ClassifyResult:
    ...
    saved, result = self._recluster_and_save(message.event_id, event)
    return self._assemble_result(message.job_id, saved, result)
  ...
  def _handle_delete(self, request: DeleteRequest) -> ClassifyResult:
    ...
    saved, result = self._recluster_and_save(request.event_id, masked)
    return self._assemble_result(request.job_id, saved, result, extra_retired=vanished)

  # ── 공통 꼬리: 재군집 + .npz rewrite + 결과 조립 ─────────────────────────────

  def _recluster_and_save(self, event_id: str, event: EventEmbeddings) -> tuple[EventEmbeddings, ReclusterResult]:
    """event 전체를 재군집하고 새 배정을 반영해 저장한다 — 이 저장이 유일한 쓰기이자 마지막 변이다.
    ...
    if not event.face_ids:
      ...
      return event, ReclusterResult(clusters=(), noise_indices=(), ambiguous_indices=(), retired_cluster_ids=())
    ...
    new_cluster_ids: list[str | None] = [None] * len(event.face_ids)  # 노이즈·ambiguous는 미배정(None)
  ...
  def _assemble_result(
    ...
    *,
    ...
    """ReclusterResult(행 인덱스 세계)를 ClassifyResult(image_id 세계)로 번역한다.

    clusters/uncertain/retired는 event 전체 스냅샷이고, common_album/failed_images는 이번
    요청분만이다 — 얼굴 미검출 사진은 .npz에 행이 없어 과거分을 복원할 수 없다 (의도된 비대칭).
    """
    clusters: list[ResultCluster] = []
    clustered_images: set[str] = set()
    for person in result.clusters:
    ...
    for reason, indices in (("ambiguous", result.ambiguous_indices), ("unmatched", result.noise_indices)):
    ...
    return ClassifyResult(
      ...
      clusters=clusters,
      common_album=list(common_album),
      uncertain=uncertain,
      # TODO(CHMO-165): 눈감음/흔들림 품질 판정 파이프라인 미구현 — 항상 빈 리스트 (options 무시)
      eyes_closed=[],
      blurry=[],
      failed_images=list(failed_images),
      retired_cluster_ids=list(dict.fromkeys([*result.retired_cluster_ids, *extra_retired])),
    )
...
if __name__ == "__main__":
  ...
  def person_vector(person: int, step: int) -> np.ndarray:
    """인물별 직교 평면 위의 단위벡터 — 같은 인물끼리 cos≥0.98, 다른 인물과 cos=0."""
  ...
  # 인물 0(A): a1·a2·a3, 인물 1(B): b1·b2, 얼굴 없는 사진 1장, 가져올 수 없는 사진 1장
  ...
  check(
    "classify: 공용 앨범·실패·부분 성공",
    result.common_album == ["img-none"]
  ...
  check("classify: .npz 저장 (행 5개)", len(store.load("event-1").face_ids) == 5)
  ...
  check(
    ...
    len(store.load("event-1").face_ids) == 5
  ...
  check(
    ...
    set(after_delete.photo_ids) == {"img-a1", "img-a2", "img-b1"}
  ...
  delete_all_body = json.dumps(
    {
      ...
      "job_id": "job-d2",
      "event_id": "event-1",
      "image_ids": ["img-a1", "img-a2", "img-b1"],
    }
  )
