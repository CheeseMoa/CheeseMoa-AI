...
`{prefix}{event_id}/{cluster_id}.jpg` 고정 (기본 prefix가 thumbnails/events/라 원본·사진 썸네일과
같은 events/{id}/ 계층) — 재군집으로 대표가 바뀌면 같은 키에 덮어쓴다
...
class S3ThumbnailStore:
  ...
  def __init__(self, s3_client, bucket: str, prefix: str = "thumbnails/events/") -> None:
...
class InMemoryThumbnailStore:
  ...
  def __init__(self, prefix: str = "thumbnails/events/") -> None:
    self._prefix = prefix
...
if __name__ == "__main__":
  ...
  if key != "thumbnails/events/event-1/person-A.jpg" or store.blobs[key] != b"jpeg-1":
