...
AuraFace (512-dim 임베딩 벡터 생성)
  │
  ▼
HDBSCAN (sklearn, eps=0.15, cosine — group 전체 임베딩 재군집)
  │
  ▼
...
| 클러스터링 | HDBSCAN (sklearn) 전체 재군집 + cluster_id 재조정 |
| 임베딩 저장 | pgvector |
| 메시지 큐 연동 | AWS SQS |
| 데이터 검증 | Pydantic v2 |
| 코드 포맷터 | Ruff |
...
### 다음 구현 목표
1. `app/pipeline/cluster.py` — HDBSCAN 전체 재군집 + cluster_id 재조정
2. `app/worker.py` — SQS consumer 워커 기본 구조 (메시지 폴링 루프)
3. `app/schemas/` — SQS 요청/결과/보정 메시지 스키마 (Spring 백엔드 연동)
