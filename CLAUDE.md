...
AuraFace (512-dim 임베딩 벡터 생성)
  │
  ▼
HDBSCAN (PoC numpy 이식본, eps=0.15, cosine — group 전체 임베딩 재군집)
  │
  ▼
...
- 재군집 뒤 결정적 후처리로 정확도 보강: 보정 제약 강제 → 파편 병합(단일 인물 파편화 교정) →
  노이즈 구제 → 저신뢰 `ambiguous` 분리(leave-one-out 유사도·마진 기준). 임계값은 전부 `ClusterConfig` 설정값
