...
- `uncertain`("분류가 어려워요") 사진은 실 `cluster_id`가 없어(.npz엔 None) 일반 reassign 대상이 못 되므로,
  ...
  미매칭 얼굴을 must-link로 인물 앨범 편입 (계약 확장, feature-spec §6.2·§6.3); 항목마다 주 인물 얼굴
  bbox 배열 `face_bboxes`(원본 px, 폭 내림차순, 빈 배열 가능)도 동봉 — 앱 상세 화면 얼굴 crop용
  (계약 교체 CHMO-407, 완료된 목표 참조)
