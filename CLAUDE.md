...
AuraFace (512-dim 임베딩 벡터 생성)
  │
  ▼
HDBSCAN (PoC numpy 이식본, cluster_selection_epsilon=0.15, cosine — group 전체 임베딩 재군집)
  │
  ▼
기존 cluster_id 재조정 (overlap 매칭으로 번호 승계, 사용자 보정은 제약)
  │
  ▼
클러스터 결과 → SQS 결과 큐로 발행 / pgvector 갱신
```
...
- 재군집 뒤 결정적 후처리로 정확도 보강: 균질 blob 승격 → 보정 제약 강제 → 파편 병합(완전 연결) →
  노이즈 구제(전역 유사도 내림차순) → 저신뢰 `ambiguous` 분리(leave-one-out 유사도·마진, 제약 당사자 보호).
  임계값은 전부 `ClusterConfig` 설정값
- 클러스터링은 전체 파이프라인 비용 0.1% 미만 (cosine은 sklearn도 brute 경로라 성능 특성 동급)

**전체 재군집 + ID 재조정 (정확도 최우선)**
- 군집의 진실은 항상 group 전체 임베딩(기존+신규)에 대한 HDBSCAN 재군집. 개별 임베딩을 pgvector에 전부 보관해 매 트리거마다 전체를 다시 군집화한다
...
| 클러스터링 | HDBSCAN (PoC numpy 이식본) 전체 재군집 + cluster_id 재조정 |
| 임베딩 저장 | pgvector |
| 메시지 큐 연동 | AWS SQS |
| 데이터 검증 | Pydantic v2 |
| 코드 포맷터 | Ruff |
...
CheeseMoa-AI/
├── app/
│   ├── worker.py            # SQS consumer 워커 엔트리포인트 (메시지 폴링 루프)
│   ├── core/                # 설정, 의존성 주입
...
- 구현 완료: `app/pipeline/detect.py`(YuNet), `app/pipeline/align.py`(Umeyama),
...
- `app/main.py`: 비어있음 (워커 엔트리포인트는 `app/worker.py`로 전환 예정)
- `healthcare_api.py`: FastAPI 학습용 샘플 코드 (실제 프로젝트 코드 아님)

### 다음 구현 목표
1. `app/worker.py` — SQS consumer 워커 기본 구조 (메시지 폴링 루프)
2. `app/schemas/` — SQS 요청/결과/보정 메시지 스키마 (Spring 백엔드 연동)
3. pgvector 저장소 계층 — 임베딩·클러스터 멤버십 저장/조회 인터페이스 (cluster.py 입출력 연결)
