...
+ 대형 근접 얼굴 재검출 회복 — rel_w≥0.30 저score(<0.6) 후보를 정규 스케일 재검출해 재검출 score≥0.80이면
되살림, YuNet이 초근접 대형 얼굴에 저score를 줘 공통첩으로 빠지던 문제, [ADR 017](docs/decisions/017-size-aware-detection-score-threshold.md)) →
...
[ADR 011](docs/decisions/011-same-photo-cannot-link.md)) → 파편 병합(완전 연결, centroid 임계 0.55
[ADR 012](docs/decisions/012-merge-threshold-recalibration.md) AND 파편 간 face-pair 평균 바닥 0.475 —
아동 교차연령 오병합 차단, [ADR 016](docs/decisions/016-merge-facepair-cohesion-gate.md)) → 노이즈
...
YuNet 5점 정렬 크롭의 눈 좌표를 경량 CNN(`open-closed-eye-0001`, OpenVINO)으로 분류하되, 판정 자격
게이트를 먼저 통과해야 한다 — bbox 짧은 변 ≥64px(그림·옆얼굴 등 정보 부족 오탐 제외) AND 눈/볼
밝기 비 ≤1.4(감은 눈꺼풀은 피부라 볼과 비슷해야 함 — 고글 반사·마스크 가림 오탐 제외,
[ADR 019](docs/decisions/019-eye-judgment-eligibility-gate.md)). 흔들림은 얼굴 crop
...
임계는 `QualityConfig`. 한계: 부분 모션블러+선명 배경 사각지대, 회전 손떨림(전역 쏠림 낮음 —
ADR 018 게이트가 해제하는 방향), 아웃포커스 주 인물(등방이라 동일), 크게 찍힌 실물 짙은 선글라스
(어두워지는 방향이라 밝기 비 상한으로 못 잡음 — 코퍼스 무표본, ADR 019 §한계), 유아 눈 CNN 오판
(도메인 문제, ADR 019 §한계), 보정 스톡 이미지의 감은 눈 한쪽 미탐(event 61 실측 closed [1.000,
0.133] — 양눈 AND 규칙 탈락, 같은 도메인 문제 계열). 웃음 캔디드 오탐은 2026-07-17 실측(859 얼굴)에서 0건 — 표정 CNN
후속은 착수 근거 없음([survey](docs/reviews/2026-07-17-smile-eyes-geometry-survey.md)).
