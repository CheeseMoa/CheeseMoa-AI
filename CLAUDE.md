...
4. 소규모 단일 인물 이벤트에서 앨범 미생성 개선 — 실사진 테스트(face-test 데이터셋)에서 확인:
   이벤트가 사실상 한 인물뿐이면 HDBSCAN이 root 클러스터를 선택하지 않아 전원 uncertain이 됨
   (같은 5장이 타인과 섞이면 정확히 묶임). `cluster._promote_single_blob`의 완전 연결 ≥0.7 조건이
   포즈 변화가 있는 실사진(동일 인물 쌍 유사도 0.46~0.70)을 구제하지 못함 — 승격 조건 완화 검토
   (feature-spec §10 #3·#4 임계값 튜닝 영역). 재현: 동일 인물 사진 4~6장만으로 classify 실행
   (로컬 검증 스크립트는 `scripts/` — .gitignore로 커밋 제외되는 로컬 디렉토리)
