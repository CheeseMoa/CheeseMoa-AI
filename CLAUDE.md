...
S3 이미지 읽기 → YuNet(얼굴 감지 + 5점 랜드마크, 배경 인물 필터 — bbox 폭이 이미지 긴 변의 1.5%
...
- **배경 인물(행인) 앨범 방지 — 얼굴 크기 필터** (2026-07-15,
  ...
  rel_w(bbox 폭/긴 변) 1.5% 하한을 추가(`DETECT_MIN_FACE_REL_WIDTH`, 0=비활성). 앨범 손실 0 +
  노이즈 얼굴 46% 제거 실측. 절대 px 기준은 저해상도 업로드를 잘라 기각.
