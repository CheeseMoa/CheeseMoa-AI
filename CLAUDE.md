...
- `app/main.py`: 비어있음 (Python 실행 환경 확인만 완료 — 워커 엔트리포인트는 `app/worker.py`로 전환 예정)
...
5. `app/pipeline/cluster.py` — HDBSCAN 전체 재군집 + cluster_id 재조정
6. `app/schemas/` — SQS 요청/결과/보정 메시지 스키마 (Spring 백엔드 연동)
