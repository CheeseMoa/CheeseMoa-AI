...
- 사용자 보정(병합/분리/이동)은 must-link/cannot-link 제약으로 반영해 재군집이 사람 결정을 뒤집지 않게 함.
  ...
  결과의 `uncertain[].album_id`에 예약 앨범 id `"__uncertain__"`을 실어 보내고 reassign의 `from_cluster_id`가
  그 값이면 해당 사진의 미매칭 얼굴을 must-link로 인물 앨범 편입한다 (계약 확장, feature-spec §6.2·§6.3)
...
### 완료된 목표
- **uncertain 사진의 인물 앨범 편입 (계약 확장)** (2026-07-04) — "분류가 어려워요"에 빠진 사진(예: 유사도
  경계로 자동 군집에서 빠진 실제 동일인물)을 사용자가 손으로 인물 앨범에 넣을 수 있게 함. uncertain 얼굴은
  실 `cluster_id`가 없어(.npz엔 None) 일반 reassign 대상이 못 되던 갭을 해결: 결과의 `uncertain[].album_id`에
  예약 앨범 id `"__uncertain__"`(`schemas/messages.py`)을 실어 보내고, reassign의 `from_cluster_id`가 그 값이면
  핸들러가 그 사진의 미매칭 얼굴을 must-link 편입한다. feedback 스키마 자체는 무변경(예약 **값**만 인식),
  `.npz`엔 여전히 None 저장(임시 id 미저장 → 재군집 은퇴 churn 회피). 실 AWS(karina 5장) end-to-end 검증 완료.
