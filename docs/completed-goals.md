...
- **Rekognition 앨범 쌍 병합 재판정 — 동일 인물 앨범 쪼개짐 회수 (P0 목표 0.2 구현분)** (2026-07-24,
  ...
  비활성 동일)·워커 스모크. 롤백: 2단 스위치 `REJUDGE_PAIR_ENABLED`(호출·판정·로그)/
  `REJUDGE_PAIR_APPLY`(반영) — **둘 다 기본 false**. 잔여: 실 이벤트 회귀(회수 12·적대 4 이벤트
  파티션 diff — 유료 호출 필요, APPLY 전환의 선행 조건), 관측 모드 며칠 운영.
