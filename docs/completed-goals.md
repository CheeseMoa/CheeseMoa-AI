...
- **미배정 얼굴 재판정의 주 인물 자격 — 배경 인물(행인) 호출 차단** (2026-07-24,
  [ADR 034](decisions/034-rejudge-main-face-gate.md),
  [분포 실측](reviews/2026-07-24-rejudge-main-face-ratio-survey.md)) — Rekognition 재판정(ADR 030)·
  비인간 게이트(ADR 032)의 미배정 판정에 크기·주인물 자격이 없어 행인까지 얼굴당 4~5콜을 받던 문제
  (실 event 147: classify 1회 121콜, 동의 없는 제3자 crop 외부 전송). 원인: 행인은 1회 등장이라
  군집이 안 돼 미배정 풀에 쌓이는데 판정 자격이 "crop 가능 여부"뿐. 해법: 그 사진 최대 얼굴폭 대비
  `main_face_ratio`(기본 0.5, ADR 022 주 인물 규칙과 동일 축·동일값) 미만인 미배정 얼굴은 crop 렌더
  이전에 스킵 — 적용은 미배정 단건 두 곳뿐(앨범 후보·앨범 쌍 판정은 대상이 주인공이라 무관). 실측
  (운영 점수 캐시 × npz 조인, 무과금, 고유 60얼굴): 편입 성공 전원 rel_photo ≥0.6이라 바닥 0.5 오컷
  0건(잘리는 고점수는 전부 같은 사진 공존 스크린 탈락분 = 가치 0 호출), 낭비 호출 60% 스킵·판정
  49% 절감. 절대 크기 축(rel_img)은 성공 0.051 vs 낭비 중앙 0.048 겹침으로 기각 — 행인은 "작은
  얼굴"이 아니라 "그 사진의 주인공이 아닌 얼굴". 검증: rejudge 70건(스킵 2종·롤백 2종·① 무관 확인)·
  worker --smoke 12건. 롤백: `REJUDGE_MAIN_FACE_RATIO=0` · `NONHUMAN_MAIN_FACE_RATIO=0`(개별),
  감시는 "배경 인물 스킵 N건" 로그 + 작게 나온 멤버 누락 리포트 시 하향이 첫 조치.
...
- **Rekognition 앨범 쌍 병합 재판정 — 동일 인물 앨범 쪼개짐 회수 (P0 목표 0.2 구현분)** (2026-07-24,
  ...
  앨범으로 갈라지는 문제(앨범 2개 이상인 34개 이벤트 중 12개=35%에 회수 가능한 쪼개짐). 원인은 파편
  병합의 face-pair 응집 바닥(ADR 016) — 회수 대상 13쌍 중 12쌍이 여기 막혔는데, 그 바닥은 정당 병합
  최강쌍이 아동 오병합 대역과 겹쳐 낮출 수 없음이 실서버 전수 채점으로 확정됐다(2026-07-23 기각
  리뷰). 얼굴 단위 재판정(ADR 030)은 미배정 얼굴만 봐서 이 구도에 닿지 않는다. 해법: 재군집 후
  로컬 게이트가 거절한 **회색지대 앨범 쌍**(centroid ≥0.35 — 비용 통제용 바닥)을 앨범당 폭최대 대표
  2장씩 K×K CompareFaces로 재판정해 **전원 ≥90 AND 산포 ≤5**일 때만 병합(`ClusterPairRejudger`).
  대표 1쌍 argmax는 실측 기각(확실한 타인 쌍이 `[99.99, 33.74, 1.96, 1.56]` — 앨범 두 개가 통째로
  융합), 같은 사진 공존(ADR 011)·사용자 cannot-link 쌍은 **호출 전** 탈락(공짜 정밀도+비용 절감),
  3개 이상 병합은 컴포넌트 완전 연결 요구(체인 융합 차단 — event 105 계열 실패 모드의 선제 방어).
  반영은 `Constraints.soft_merge` + `_apply_soft_merge`로 **모든 응집 게이트 이후** 두 대표의 최종
  클러스터를 union — must-link 강제는 대상 앨범의 응집을 떨어뜨려 재파편화·축출을 부르는 event 134
  회귀와 같은 함정이라 금지(`soft_attach`보다 먼저 적용해야 어태치가 최종 라벨을 본다). 영속은
  `.npz` v6 `soft_merge_pairs`(v5 이하 빈 튜플 폴백) — 캐시가 유실돼도 앨범이 도로 쪼개지지 않는
  재전달 안전성. 점수 캐시는 ADR 030과 공유하고 **키 방향을 face_id 사전순으로 고정**해(구현 시
  확정) 재군집마다 방향이 뒤집혀 재과금되는 함정을 막았다. 두 재판정은 1차 패스 클러스터 위에서
  함께 판정하고 2차 패스 한 번에 반영한다. 검증(AWS 0원): rejudge 45건(체인 차단 3종·산포·같은사진
  0호출·캐시 히트)·cluster 51건(순서 계약·붕괴 remap)·event_embeddings 32건(v6 왕복·v5 폴백)·
  handlers 76건 종단(병합→저장→재분류 0호출 유지·사용자 split이 되돌림·관측 모드 불변·전면 장애 =
  비활성 동일)·워커 스모크. 롤백: 2단 스위치 `REJUDGE_PAIR_APPLY=false`(새 병합만 차단, 판정 로그는
  유지)/`REJUDGE_PAIR_ENABLED=false`(호출 0회, 완전 원복) — **둘 다 기본 true로 활성화**(2026-07-24
  결정). 잔여: 실 이벤트 회귀(회수 12·적대 4 이벤트 파티션 diff — 유료 호출 필요)를 **활성화보다
  늦게** 사후 수행, 그 전까지는 초기 운영 감시(앨범 수 급감·산포 큰 병합 로그·병합 직후 split)가
  대역이다. 이미 저장된 `soft_merge_pairs`는 APPLY=false로도 유지되므로 완전 원복은 `.npz` 비우기 +
  재분류(개별 앨범은 사용자 split이 되돌림).
