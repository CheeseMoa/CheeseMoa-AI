...
- 보정 강제 뒤 **정확도 보강 후처리 3종**을 순서대로 적용한다 (임계는 전부 `ClusterConfig` 설정값):
  1. **파편 병합** — centroid 코사인 유사도가 동일 인물 수준(기본 0.7) 이상인 클러스터끼리 병합.
     `allow_single_cluster=False` 특성상 단일 인물 위주 group이 파편화되는 케이스를 교정한다.
     cannot-link로 연결된 클러스터 쌍은 병합하지 않는다(사용자 분리 결정 보존)
  2. **노이즈 구제** — 최근접 centroid 유사도가 기본 0.6 이상인 노이즈 얼굴을 그 클러스터에 편입
  3. **저신뢰 분리** (TBD #3 기본 정책) — leave-one-out centroid 유사도가 바닥(기본 0.4) 미만이면
     노이즈로 강등, 2위 클러스터와의 마진이 기본 0.05 미만이면 `ambiguous`로 분리. must-link로
     확정된 얼굴은 보호하고, cannot-link로 연결된 클러스터 쌍은 마진 비교에서 서로 제외한다
...
> 단일 인물 group 파편화 엣지(ADR 005)는 파편 병합 후처리로 교정됨을 합성 데이터로 검증했다
> (1인물 20장: `[14, 2]+노이즈 4` → 1클러스터 20장). `allow_single_cluster=True`는
> `eps=0.15`와의 상호작용으로 전원 노이즈가 되어 해법이 아님도 실험으로 확인.
