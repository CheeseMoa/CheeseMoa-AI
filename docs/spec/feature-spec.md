...
```jsonc
{
  ...
  "common_album": ["uuid"],     // common 앨범 — 특정 인물 귀속 불가(단체·배경·얼굴 미검출). 뷰어 노출
...
- `uncertain`: 인물에 자신 있게 못 붙인 사진("분류가 어려워요", 내부 검수용). `reason`은
  ...
  **얼굴 미검출(인물 없는) 사진은 uncertain이 아니라 `common_album`으로 확정**(결정 2026-07-03).
  - **미매칭 사진의 단체/행인 구분**(결정 2026-07-03): 검출 얼굴이 **2개 이상인데 아무 얼굴도 인물에
    매칭되지 않은 사진은 단체·배경 사진으로 보고 `common_album`**으로, 얼굴 1개(행인·미등록 1인) 사진만
    `uncertain(unmatched)`으로 보낸다. 얼굴이 하나라도 인물에 매칭되면 해당 인물 앨범이 우선한다(다대다).
...
| 단체·배경·인물 없는 사진 = 공통 사진첩 | 특정 인물 귀속 불가 사진(얼굴 미검출 포함)은 `common_album`으로 |
...
> - **눈감음**: YuNet 5점으로 정렬한 크롭의 고정 눈 좌표에서 양눈을 잘라 경량 CNN(open-closed-eye-0001)으로
>   분류, 양눈 모두 감김이면 그 얼굴이 눈감음. 임계는 `eye_closed_confidence`(설정값). 판정 자격 게이트
>   ([ADR 019](../decisions/019-eye-judgment-eligibility-gate.md)): bbox 짧은 변 ≥64px AND 눈/볼 밝기 비
>   ≤1.4를 통과한 얼굴만 판정 — 초소형 그림 얼굴·고글/마스크 가림이 실측 오탐의 주 유형이라 CNN 앞에서
>   거른다. *한계*: 대형 실물 짙은 선글라스(무표본 미검증)·유아 눈 CNN 오판(도메인 문제)은 남는다.
>   웃다가 접힌 눈(캔디드) 오탐은 2026-07-17 실측 859 얼굴에서 0건 — 표정 CNN 후속은 착수 근거 없음
>   ([survey](../reviews/2026-07-17-smile-eyes-geometry-survey.md)).
