...
- `uncertain`: 인물에 자신 있게 못 붙인 사진("분류가 어려워요", 내부 검수용). `reason`은
  ...
  **얼굴 미검출(인물 없는) 사진은 uncertain이 아니라 `common_album`으로 확정**(결정 2026-07-03).
  - **비인간 얼굴 강등 사진도 공용 앨범**(결정 2026-07-24, [ADR 032](../decisions/032-nonhuman-face-gate.md),
    `NONHUMAN_GATE_ENABLED=true` 기본): 인형·조형물로 판정돼 강등된 얼굴은 재군집·머릿수에서 빠지고,
    사진의 **모든** 얼굴이 강등이면 "얼굴 0개" 경로와 같은 목적지인 `common_album`으로 보낸다(event
    스코프 — 강등 기록이 `.npz`에 남아 과거분도 복원된다). 실인물 얼굴이 하나라도 남으면 그 얼굴
    기준으로 종전 라우팅 그대로다(아이 1명 + 인형 1개 사진은 1인 사진으로 취급).
  - **단체 사진 = 공용 앨범**(결정 2026-07-20, `CLUSTER_GROUP_PHOTO_TO_COMMON=true` 기본): **주 인물
    얼굴이 2개 이상인 사진은 매칭 여부와 무관하게 `common_album`에 노출**한다 — 단체 사진은 그 자리에
    함께 있던 모두의 사진이라는 제품 결정. 얼굴이 인물에 매칭되면 해당 인물 앨범에도 함께 들어간다(다대다,
    공용과 인물 앨범 **중복 노출**). 전원 미매칭에 주 인물 1개(초상·미등록 1인)인 사진은 `uncertain(unmatched)`으로
    보낸다(매칭 얼굴이 있는 사진의 미매칭 주 인물은 아래 결정 2026-07-21 참조).
    - **주 인물 판정**(결정 2026-07-20): 그 사진 최대 얼굴 폭의 50%(`CLUSTER_COMMON_MAIN_FACE_RATIO`,
      0=전체 카운트) 미만 얼굴은 지나가는 행인으로 보고 세지 않는다 — 품질 게이트의 주 인물 규칙(ADR 022)과
      동일 논리. 1인 인물 사진 + 배경 행인이 단체 사진으로 오인돼 공용에 가는 것을 막는다. 판정 입력인
      얼굴 bbox 폭은 `.npz` v2의 `face_widths` 열이며, v1 파일은 폭 미상(0)으로 로드돼 전원 주 인물
      취급 — 종전(전체 얼굴 수) 동작과 동일한 안전 폴백(사진 단위로 세대가 갈려 혼합 없음).
    - **OR 인물 앨범 2개+ 사진도 단체**(결정 2026-07-20): 서로 다른 인물 앨범 2개 이상에 속한 사진은 주
      인물 수와 무관하게 공용에도 노출한다 — 뒤에 작게 찍혔어도 인식된 일행이면 행인이 아니라서 크기
      게이트가 놓치는 케이스. 이 조건은 Spring의 종전 "여러 인물 앨범 중복 사진 → 공용 복제" 규칙과
      정확히 같다 → **이 정책(`CLUSTER_GROUP_PHOTO_TO_COMMON=true`)이 켜져 있는 한 AI의 `common_album`이
      그 규칙의 상위집합이므로 백엔드 복제 로직은 제거해도 된다.** 단 제거 후에는 이 토글을 false로
      롤백하면 공용 중복 노출이 통째로 사라지므로(백엔드 복제도 없음), 롤백 시 백엔드 로직 복원이 함께
      필요하다 — 토글은 상시 ON을 전제로 한다.
    ...
    - **Spring 복제 로직과의 관계**: Spring의 종전 "여러 인물 앨범 중복 사진 → `common` 복제" 로직은
      바로 아래 OR 조건으로 AI에 이식됐다 — 겹쳐 있어도 공용 멤버십은 집합이라 멱등·무해하고, 이
      정책이 켜져 있는 한 AI가 상위집합이므로 백엔드 쪽은 제거해도 된다(아래 OR 조건의 롤백 주의 참조).
...
- `thumbnail_s3_key`(계약 확장, CHMO-335): 인물 앨범 목록용 **대표 얼굴 썸네일**의 S3 키. 워커가 재군집
  ...
  (`.npz` v2 이하 — bbox·원본 키 미상) 데이터만의 클러스터. crop 입력인 bbox·원본 s3_key는 `.npz` v3 열이다.
