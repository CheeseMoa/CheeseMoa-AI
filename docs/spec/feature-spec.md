...
- **인물 식별의 연속성은 AI 몫**: AI가 전체 재군집 후 **overlap 재조정으로 기존 `cluster_id`를 승계**해 같은 사람을 업로드 간 같은 번호로 유지해 보낸다. Spring은 `cluster_id` ↔ 이름(예: "민준") 매핑만 담당하며 **AI는 이름을 모른다**. 즉 사진을 사람별로 묶는 일은 AI가 끝내고, Spring은 번호표에 이름표만 붙인다(Spring이 다시 군집하지 않음).
