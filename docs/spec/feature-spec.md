...
```jsonc
{
  ...
  "uncertain": [                // uncertain("분류가 어려워요") — 뷰어 비노출
    { "image_id": "uuid", "reason": "ambiguous" }   // "ambiguous"(저신뢰) | "unmatched"(인물 미매칭)
  ],
...
>   분류, 양눈 모두 감김이면 그 얼굴이 눈감음. 임계는 `eye_closed_confidence`(설정값). *한계*: 웃다가 접힌 눈
>   (캔디드)도 감김으로 잡힌다 — 표정 분리는 5점 기하로 불가하며 별도 표정 CNN이 필요해 보류.
